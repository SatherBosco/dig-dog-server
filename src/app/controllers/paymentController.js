    ...
    try {
        ...
        if (withdrawDate !== null) {
            if (!withdrawDate.paid) {
                if (new Date(withdrawDate.createdAt).getTime() + 1200000 > new Date().getTime()) {
                    // ADICIONAR VARIAVEL PARA A DATA
                    if (new Date(withdrawDate.createdAt).getTime() + 600000 > new Date().getTime()) {
                        // ADICIONAR VARIAVEL PARA A DATA
                ...
                if (new Date(withdrawDate.createdAt).getTime() + 48 * 3600000 > new Date().getTime()) {
                    // ADICIONAR VARIAVEL PARA A DATA
        ...
        if (withdrawReq.bone < 400) return res.status(400).send({ msg: "Saque mínimo de 400 Bone." });
        if (account.bone - withdrawReq.bone < 100) return res.status(400).send({ msg: "É necessário deixar no mínimo 100 Bone na conta." });
        ...
        const _contract = "0x742FAC2431ae79A5cf2F73A3EeB67f740411b326";
        const SmartContractWithdrawObj = new ethers.Contract(_contract, SmartContractWithdraw, provider);
        ...
        let withdrawTransaction = await SmartContractWithdrawObj.getMessage(_recipient, _amount, _transactionId, _date, _contract);
